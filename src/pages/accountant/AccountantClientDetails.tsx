...
import { invoiceStatusColors, invoiceStatusLabels } from '@/lib/invoice-status-config';
import {
    AccountantDocument,
...
const currentMonthPeriod = `m${String(new Date().getMonth() + 1).padStart(2, '0')}` as AccountantDocumentPeriod;
...
export function AccountantClientDetails() {
    ...
    const [activeTab, setActiveTab] = useState<AccountantDocumentType>('invoices');
    const [selectedYear, setSelectedYear] = useState(currentYear);
    const [selectedPeriod, setSelectedPeriod] = useState<AccountantMonthPeriod>(currentMonthPeriod as AccountantMonthPeriod);
    const [selectedStatuses, setSelectedStatuses] = useState<AccountantDocumentStatus[]>([]);
    const [page, setPage] = useState(1);
    const [documents, setDocuments] = useState<PaginatedDocuments>(emptyDocuments);
    ...
    const selectedDownloadableDocuments = useMemo(
        () => documents.data.filter((document) => selectedIds.has(document.id) && document.storage_available),
    ...
    const allSelectableChecked = hasSelectableRows && selectedOnPageCount === selectableIds.length;
    const partiallySelected = selectedOnPageCount > 0 && !allSelectableChecked;
    const canDownloadFilteredDocuments = documents.downloadable_total > 0 && documents.downloadable_total <= 100;
    const noDownloadableFilteredDocuments = documents.downloadable_total === 0;
    const tooManyDownloadableFilteredDocuments = documents.downloadable_total > 100;
    ...
    useEffect(() => {
        const loadClient = async () => {
            ...
            setLoading(true);
            try {
                const clients = await companyService.getLinkedClients(cabinetCompany.id);
                const found = (clients as LinkedClientWithStats[]).find((company) => company.id === clientId);
    ...
    const selectedStatusesLabel = useMemo(() => {
        ...
        if (selectedStatuses.length === 1) {
            return statusOptions.find((status) => status.value === selectedStatuses[0])?.label || 'Statut';
        }
    ...
    useEffect(() => {
        ...
        void loadDocuments();
    }, [cabinetCompany?.id, clientId, activeTab, page, selectedYear, selectedPeriod, selectedStatuses, companiesLoading]);
    ...
    const handleDownload = async (document: AccountantDocument) => {
        ...
        try {
            ...
            anchor.href = url;
            anchor.download = document.downloadable_filename || `document-${document.invoice_number}.pdf`;
            ...
            toast({
                ...
                description: error.message || 'Le document comptable n’a pas pu être téléchargé.',
            });
        } finally {
    ...
    const handleBulkDownload = async () => {
        ...
        if (selectedDownloadableDocuments.length === 0 && noDownloadableFilteredDocuments) {
            toast({
                variant: 'destructive',
        ...
        if (selectedDownloadableDocuments.length === 0 && tooManyDownloadableFilteredDocuments) {
            toast({
                variant: 'destructive',
                title: 'Affinez les filtres',
                description: 'Le téléchargement groupé est limité à 100 documents archivés.',
            });
            return;
        }
        ...
        try {
            const blob = await companyService.downloadLinkedClientDocumentsZip(
                ...
                clientId,
                {
                    document_ids: selectedDownloadableDocuments.length > 0
            ...
            toast({
                ...
                description: error.message || 'Le ZIP comptable n’a pas pu être généré.',
            });
        } finally {
    ...
    const kpis = [
        { label: 'CA annuel', value: formatCurrency(client.stats?.annual_revenue || 0), icon: Euro, color: 'text-blue-600' },
        { label: 'Payé', value: formatCurrency(client.stats?.total_paid || 0), icon: CheckCircle, color: 'text-green-600' },
        { label: 'En attente', value: formatCurrency(client.stats?.pending_amount || 0), icon: Clock, color: 'text-yellow-600' },
        { label: 'En retard', value: formatCurrency(client.stats?.overdue_amount || 0), icon: AlertTriangle, color: 'text-red-600' },
    ];

    return (
        <div className="space-y-6">
            <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:gap-4">
                <Button variant="ghost" size="sm" onClick={() => navigate('/accountant')}>
                    <ArrowLeft className="mr-2 h-4 w-4" /> Retour
                </Button>
                <div className="min-w-0">
                    <h1 className="break-words text-2xl font-bold">{client.name}</h1>
                    <p className="text-sm text-muted-foreground">
                        Pièces comptables immuables du dossier client, filtrées par année, mois et statut.
                    </p>
                    {client.siren && (
                ...
                <CardContent className="flex flex-col gap-4 pt-6 xl:flex-row xl:items-end xl:justify-between">
                    ...
                    <div className="flex min-w-0 flex-1 flex-wrap items-end justify-end gap-x-3 gap-y-2">
                        <div className="w-full min-w-0 shrink-0 space-y-1 sm:w-[9.5rem] sm:max-w-full">
                            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Année</p>
                            <Select
                        ...
                        <div className="w-full min-w-0 shrink-0 space-y-1 sm:w-44 sm:max-w-full">
                            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Mois</p>
                            <Select
                                value={selectedPeriod}
                                onValueChange={(value) => setSelectedPeriod(value as AccountantMonthPeriod)}
                            >
                                <SelectTrigger className="h-9 w-full">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as AccountantDocumentType)}>
                <TabsList className="grid h-auto w-full grid-cols-2 gap-1">
                    <TabsTrigger value="invoices" className="w-full gap-2 whitespace-normal px-3 py-2 text-xs sm:text-sm">
                        <Receipt className="h-4 w-4" />
                        <span className="min-w-0 truncate">Factures ({client.stats?.invoice_count || 0})</span>
                    </TabsTrigger>
                    <TabsTrigger value="credit-notes" className="w-full gap-2 whitespace-normal px-3 py-2 text-xs sm:text-sm">
                        <FileMinus className="h-4 w-4" />
                        <span className="min-w-0 truncate">Avoirs ({client.stats?.credit_note_count || 0})</span>
                    </TabsTrigger>
                </TabsList>

                {(['invoices', 'credit-notes'] as AccountantDocumentType[]).map((tab) => (
                    <TabsContent key={tab} value={tab}>
                        <Card>
                            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div className="min-w-0">
                                    <CardTitle className="text-base">
                                        {tab === 'invoices' ? 'Factures archivées' : 'Avoirs archivés'}
                                    </CardTitle>
                                    <p className="text-sm text-muted-foreground">
                                        {documents.total} pièce(s), dont {documents.downloadable_total} ZIPpable(s) pour les filtres actifs.
                                    </p>
                                </div>
                                <div className="flex w-full flex-col items-start gap-2 sm:w-auto sm:items-end">
                                    <div className="flex flex-wrap items-center gap-2">
                                        <Badge variant="outline" className="w-fit">
                                            {selectedPeriodOption?.label || selectedPeriod} {selectedYear}
                                        </Badge>
                                    ...
                                    {tooManyDownloadableFilteredDocuments && selectedDownloadableDocuments.length === 0 && (
                                        <p className="text-xs text-muted-foreground">
                                            Affinez les filtres pour rester sous 100 documents archivés.
                                        </p>
                                    )}
                                        ...
                                        <Table className="min-w-[760px]">
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    <TableHead className="whitespace-nowrap">Archive PDF</TableHead>
                                                ...
                                                {documents.data.map((document) => (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="whitespace-nowrap">
                                                            {formatDate(document.issue_date || document.created_at)}
                                                        </TableCell>
                                                        <TableCell className="whitespace-nowrap text-right">
                                                            {formatCurrency(Math.abs(Number(document.total || 0)))}
                                                        </TableCell>
                                                        <TableCell className="whitespace-nowrap">
                                                            <Badge className={invoiceStatusColors[document.status]}>
                                                        ...
                                                        <TableCell className="whitespace-nowrap text-right">
                                                            <Button
                                                                ...
                                                                disabled={!document.storage_available || downloadingId === document.id}
